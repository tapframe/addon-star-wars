...
const outputNewRepublicPath = path.join(__dirname, '../Data/newRepublicEraData.js'); // Updated output path

// --- List of Star Wars New Republic Era Series to Fetch ---
const seriesToFetch = [
    ...
    { title: "Skeleton Crew", year: 2024, type: 'series' } // May not have full details yet
];
...
async function fetchNewRepublicEraSeries() { // Renamed function
    console.log('🚀 Starting Star Wars New Republic Era Series data fetching process...'); // Updated log
    const finalSeriesData = [];
    let processedCount = 0;

    for (const series of seriesToFetch) {
        processedCount++;
        console.log(`\nProcessing New Republic Era Series ${processedCount}/${seriesToFetch.length}: \"${series.title}\" (${series.year})...`); // Updated log
    ...
    writeDataFile(outputNewRepublicPath, finalSeriesData, "Star Wars New Republic Era Series"); // Updated log
    console.log('\n✨ Star Wars New Republic Era Series data fetching complete!'); // Updated log
}

// --- Run Script ---

fetchNewRepublicEraSeries().catch(err => { // Renamed function call
