...
const outputEmpireEraPath = path.join(__dirname, '../Data/empireEraData.js'); // Updated output path

// --- List of Empire Era Content to Fetch ---
...
// --- Helper Functions (Adapted for Series/Movies) ---
...
async function fetchEmpireEraData() { // Renamed function
    console.log('🚀 Starting Empire Era content data fetching process...'); // Updated log
    const finalContentData = [];
    let processedCount = 0;

    for (const item of contentToFetch) {
        processedCount++;
        console.log(`\nProcessing Empire Era item ${processedCount}/${contentToFetch.length}: \"${item.title}\" (${item.year}, ${item.type})...`); // Updated log
    ...
    finalContentData.sort(sortByReleaseYear);
    writeDataFile(outputEmpireEraPath, finalContentData, "Empire Era Content"); // Updated log
    console.log('\n✨ Empire Era content data fetching complete!'); // Updated log
}

// --- Run Script ---

fetchEmpireEraData().catch(err => { // Renamed function call
