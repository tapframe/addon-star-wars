...
const outputDroidsCreaturesPath = path.join(__dirname, '../Data/droidsCreaturesData.js'); // Updated output path

// --- List of Droids & Creatures Series/Shorts to Fetch ---
const seriesToFetch = [
...
async function fetchDroidsCreaturesSeries() { // Renamed function
    console.log('🚀 Starting Droids & Creatures Series/Shorts data fetching process...'); // Updated log
    const finalSeriesData = [];
    let processedCount = 0;

    for (const series of seriesToFetch) {
        processedCount++;
        console.log(`\nProcessing Droid/Creature item ${processedCount}/${seriesToFetch.length}: \"${series.title}\" (${series.year})...`); // Updated log
        ...
        if (!details.poster_path || !details.overview || details.overview.trim() === '') {
            ...
            // Could relax this for shorts, but let's keep it strict for now
            continue;
        }
    ...
    writeDataFile(outputDroidsCreaturesPath, finalSeriesData, "Droids & Creatures Series/Shorts"); // Updated log
    console.log('\n✨ Droids & Creatures Series/Shorts data fetching complete!'); // Updated log
}

// --- Run Script ---

fetchDroidsCreaturesSeries().catch(err => { // Renamed function call
