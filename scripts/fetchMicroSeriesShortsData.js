...
const outputMicroSeriesPath = path.join(__dirname, '../Data/microSeriesShortsData.js'); // Updated output path

// --- List of Star Wars Micro-Series & Shorts to Fetch ---
const seriesToFetch = [
    { title: "Star Wars: Clone Wars", year: 2003, type: 'series' }, // Genndy Tartakovsky version
    { title: "Star Wars Blips", year: 2017, type: 'series' },
    { title: "Star Wars Forces of Destiny", year: 2017, type: 'series' }, // Adjusted title slightly
    { title: "Star Wars Galaxy of Adventures", year: 2018, type: 'series' },
    { title: "Star Wars Roll Out", year: 2019, type: 'series' },
    { title: "Star Wars Galaxy of Creatures", year: 2021, type: 'series' },
    { title: "Star Wars Galactic Pals", year: 2022, type: 'series' },
    { title: "Zen - Grogu and Dust Bunnies", year: 2022, type: 'series' }, // TMDB lists as TV short
    { title: "Fun with Nubs", year: 2024, type: 'series' } // May not be found yet
];
...
async function searchTmdb(title, year, type) {
    ...
    try {
        ...
        if (res.data && res.data.results && res.data.results.length > 0) {
            ...
            // For shorts, sometimes the exact year match is less critical if the title is specific enough
...
async function fetchStarWarsMicroSeries() { // Renamed function
    console.log('🚀 Starting Star Wars Micro-Series & Shorts data fetching process...'); // Updated log
    const finalSeriesData = [];
    let processedCount = 0;

    for (const series of seriesToFetch) {
        processedCount++;
        console.log(`\nProcessing Micro-Series/Short ${processedCount}/${seriesToFetch.length}: \"${series.title}\" (${series.year})...`); // Updated log
        ...
        if (!details.poster_path || !details.overview || details.overview.trim() === '') {
            // For very short series/blips, overview might be less critical, but poster is usually good to have
    ...
    writeDataFile(outputMicroSeriesPath, finalSeriesData, "Star Wars Micro-Series & Shorts"); // Updated log
    console.log('\n✨ Star Wars Micro-Series & Shorts data fetching complete!'); // Updated log
}

// --- Run Script ---

fetchStarWarsMicroSeries().catch(err => { // Renamed function call
