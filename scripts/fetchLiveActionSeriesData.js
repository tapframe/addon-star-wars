...
const outputLiveActionSeriesPath = path.join(__dirname, '../Data/liveActionSeriesData.js'); // Updated output path

// --- List of Star Wars Live-Action Series to Fetch ---
const seriesToFetch = [
    ...
    { title: "Skeleton Crew", year: 2024, type: 'series' }
];


// --- Helper Functions (Adapted from fetchAnthologyFilmsData.js for consistency) ---

async function searchTmdb(title, year, type) {
    const mediaType = type === 'movie' ? 'movie' : 'tv'; // TMDB uses 'tv' for series
...
async function fetchStarWarsLiveActionSeries() { // Renamed function
    console.log('🚀 Starting Star Wars Live-Action Series data fetching process...'); // Updated log
    const finalSeriesData = [];
    let processedCount = 0;

    for (const series of seriesToFetch) {
        processedCount++;
        console.log(`\nProcessing Live-Action Series ${processedCount}/${seriesToFetch.length}: \"${series.title}\" (${series.year})...`); // Updated log
        ...
        const title = (details.name || series.title).trim(); // Use details.name for series title
    ...
    writeDataFile(outputLiveActionSeriesPath, finalSeriesData, "Star Wars Live-Action Series"); // Updated log
    console.log('\n✨ Star Wars Live-Action Series data fetching complete!'); // Updated log
}

// --- Run Script ---

fetchStarWarsLiveActionSeries().catch(err => { // Renamed function call
